...
import { ContactlessCheckInModal } from '../../components/ContactlessCheckInModal';
...
export const GuestStaysTab: React.FC<GuestStaysTabProps> = ({ onOpenMobileKey, onRequestService }) => {
  ...
  const [checkInModalStay, setCheckInModalStay] = useState<any | null>(null);
  ...
  const [upcomingStays, setUpcomingStays] = useState([
    {
      id: 'res_up_01',
      propertyName: 'Cedar & Salt Desert Hub',
      propertyCity: 'Moab',
      propertyState: 'UT',
      roomTypeName: 'Desert Cliff King Suite',
      confirmationCode: 'CS-551029',
  ...
  return (
    <div className="space-y-8 font-sans">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {upcomingStays.map((stay) => (
            ...
            >
              <div className="flex items-start justify-between">
                ...
                <span className={`px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider border ${
                  stay.isCheckInOpen
                    ...
                    : 'bg-[#F3EDF8] text-[#4A1D6D] border border-[#E2D4F0]'
                }`}>
                  {stay.isCheckInOpen ? 'Arriving Today • Check-In Ready' : `In ${stay.daysAway} Days`}
                </span>
              </div>
              ...
              <div className="flex items-center justify-between pt-1 border-t border-[#E9E5EE] gap-2">
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="px-2.5 py-1.5 rounded-lg bg-white hover:bg-[#FAF9FC] text-[#6E6678] hover:text-[#1E1627] font-semibold text-xs border border-[#E9E5EE] transition cursor-pointer flex items-center gap-1 shadow-2xs"
                  >
                    <Receipt className="w-3.5 h-3.5" /> View Folio
                  </button>
                  <button
                    type="button"
                    onClick={() => setCheckInModalStay(stay)}
                    className={`px-3 py-1.5 rounded-xl font-bold text-xs transition cursor-pointer flex items-center gap-1.5 shadow-2xs ${
                      stay.isCheckInOpen
                        ...
                        : 'bg-[#4A1D6D] hover:bg-[#3B1457] text-white'
                    }`}
                  >
                    <Key className="w-3.5 h-3.5" />
                    <span>{stay.isCheckInOpen ? 'Express Check-In' : 'Pre-Register'}</span>
                  </button>
                </div>
              </div>
            </div>
        ...
        <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 animate-fadeIn">
          <div className="bg-white rounded-3xl border border-[#E9E5EE] max-w-md w-full p-6 space-y-5 shadow-2xl">
            ...
            <button
              type="button"
              onClick={() => setSelectedFolioModal(null)}
              className="w-full py-2.5 rounded-xl bg-[#4A1D6D] hover:bg-[#3B1457] text-white text-xs font-semibold transition cursor-pointer"
            >
              Close Folio Summary
            </button>
          </div>
      ...
      {/* 4. Contactless Pre-Arrival Check-In Modal */}
      {checkInModalStay && (
        <ContactlessCheckInModal
          reservation={checkInModalStay}
          onClose={() => setCheckInModalStay(null)}
          onSuccess={(checkedInStay) => {
            setActiveStay({
              id: checkedInStay.id,
              propertyName: checkedInStay.propertyName,
              propertyCity: checkedInStay.propertyCity || 'Moab',
              propertyState: checkedInStay.propertyState || 'UT',
              roomNumber: checkedInStay.roomNumber || '204',
              roomTypeName: checkedInStay.roomTypeName,
              confirmationCode: checkedInStay.confirmationCode,
              checkInDate: checkedInStay.checkInDate,
              checkOutDate: checkedInStay.checkOutDate,
              nights: checkedInStay.nights || 3,
              guests: checkedInStay.guests || '2 Adults',
              keyStatus: 'Active & Provisioned',
              status: 'In-House',
              wifiNetwork: 'LumenStay-Guest',
              wifiPassword: 'Sanctuary2026',
              totalAmount: checkedInStay.totalAmount || 1045.0,
            });
            setUpcomingStays((prev) => prev.filter((s) => s.id !== checkedInStay.id));
            setCheckInModalStay(null);
            onOpenMobileKey();
          }}
        />
      )}
